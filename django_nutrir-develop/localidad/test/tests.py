...
@pytest.mark.django_db
def test_Localidades_DeptoProvincia_sin_loguearse(client_unlogged):
    # Get urls
    localidad = LocalidadFactory()
    api_url = reverse('LocalidadDepartamentoProvincia',kwargs={'id_p': localidad.provincia.id, 'id_d': localidad.departamento.id})
...
@pytest.mark.django_db
def test_localidad_DeptoProvincia_logueado(client_logged):
    # SetUp
    localidad = LocalidadFactory()
    ...
    assert response.status_code == status.HTTP_200_OK
    assert len(response.data) == 1
    assert response.data[0]['provincia'] == localidad.provincia.id and response.data[0]['departamento'] == localidad.departamento.id
